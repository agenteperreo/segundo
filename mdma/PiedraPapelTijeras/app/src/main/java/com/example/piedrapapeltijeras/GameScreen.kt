            ...
            onGameEnd = {
                ...
                if (machineScore == 3) {
                    ...
                    GlobalScope.launch(Dispatchers.IO) {
                        val player = database.PlayerDao().getPlayerStatsByName(username)
                        // Incrementa las partidas jugadas
                        player.partidasJugadas += 1
                        // Actualiza la máxima puntuación si es mayor que la existente
                        if (playerScore > player.maxPuntuacion) {
                            player.maxPuntuacion = playerScore


                            // Guarda los cambios en la base de datos
                            database.PlayerDao().updatePlayerStats(player)
                        }
                    }
...
fun GameScreenPreview() {
    GameScreen(
        ...
        username = "username",
        database = PlayerDatabase.getInstance(LocalContext.current)
    )
}
